...
from isaaclab.managers import ManagerBase, ManagerTermBase, TerminationTermCfg
...
from isaaclab_mimic.envs.manager_based_rl_mimic_env import ManagerBasedRLMimicEnv
...
class DataGenerator:
    ...
    def __init__(
        ...
        self.default_actions = self.env_cfg.mimic_config.default_actions.to(
            self.env.device
        ).repeat((self.env.num_envs, 1))
        self.warmup_steps = self.env_cfg.mimic_config.warmup_steps
        ...
        self.current_eef_subtask_indices = {}
        self.current_eef_subtask_step_indices = {}
        self.current_eef_subtask_trajectories = {}  # just for wbc now
        self.randomized_subtask_boundaries = {}
        self.runtime_subtask_constraints_dict = {}
        self.current_eef_selected_src_demo_indices = {}
        self.env.register_data_generator(self)
    ...
    def generate_trajectory(
        ...
        lgr.trace("Selected source demo index:", selected_src_demo_ind)
        ...
        if use_delta_transform is not None:
            ...
            if coord_transform_scheme is not None:
                ...
                if subtask_object_name is not None:
                    ...
                    lgr.trace(f"skipping transformation for {subtask_object_name}")
        ...
        if hasattr(
            ...
            "clip_src_traj_strategy",
        ):
            clipper_cfg = self.env_cfg.subtask_configs[eef_name][
                subtask_ind
            ].clip_src_traj_strategy
            clipper = clipper_cfg.class_type(clipper_cfg)
            transformed_eef_poses, src_subtask_gripper_actions = clipper.cilp_src_traj(
                eef_pose=self.env.get_robot_eef_pose(
                    env_ids=[env_id], eef_name=eef_name
                )[0],
                object_pose=subtask_object_pose,
                src_traj=transformed_eef_poses,
                src_gripper_actions=src_subtask_gripper_actions,
            )
        ...
        if not wbc:
            if (
                ...
                init_sequence = WaypointSequence.from_poses(
                    poses=self.env.get_robot_eef_pose(
                        env_ids=[env_id], eef_name=eef_name
                    )[0].unsqueeze(0),
    ...
    async def generate(
        ...
        """
        assert use_mp is False, "use_mp is not supported yet"
        ...
        while True:
            async with self.src_demo_datagen_info_pool.asyncio_lock:
                ...
                for (
                    ...
                    if eef_subtask_step_index is None:
                        ...
                        self.current_eef_subtask_step_indices[env_id][eef_name] = 0
            ...
            for eef_name in self.env_cfg.subtask_configs.keys():
                ...
                subtask_ind = self.current_eef_subtask_indices[env_id][eef_name]
                if self.current_eef_subtask_step_indices[env_id][eef_name] == len(
                    current_eef_subtask_trajectories[eef_name]
                ):  # subtask done
                    if (eef_name, subtask_ind) in runtime_subtask_constraints_dict:
                        task_constraint = runtime_subtask_constraints_dict[
                        ...
                        if (
                            ...
                            assert (
                                ...
                                or self.current_eef_subtask_step_indices[env_id][
                                    concurrent_task_spec_key
                                ]
                                >= len(
                                    current_eef_subtask_trajectories[
            ...
            if all(eef_subtasks_done.values()) or generated_success:
                break
