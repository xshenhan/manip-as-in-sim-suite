...
def get_parameter_input(
    ...
    if isinstance(current_val, (tuple, list)):
        ...
        full_param_name = (
            f"{event_term_name}.{param_name}" if event_term_name else param_name
        )
    ...
    for key, allowed_range in param_config.items():
        ...
        if isinstance(allowed_range, dict):
            interactive_update_randomizable_params(
                event_term, event_term_name, allowed_range, current_path, env
            )
        else:
            try:
...
def setup_output_paths(output_file_path: str) -> tuple[str, str, str, str]:
    ...
    output_file_name = output_file_path.stem
    output_failed_file_name = output_file_name + "_failed"
    ext = output_file_path.suffix
    # create directory if it does not exist
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    return output_dir, output_file_name, output_failed_file_name, ext
