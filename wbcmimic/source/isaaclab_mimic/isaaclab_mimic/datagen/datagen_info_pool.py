...
from isaaclab_mimic.envs import KeyPointSubTaskConfig, RLWbcSubTaskConfig
...
class DataGenInfoPool:
    ...
    def __init__(self, env, env_cfg, device, asyncio_lock: asyncio.Lock | None = None):
        ...
        for eef_name, eef_subtask_configs in env_cfg.subtask_configs.items():
            ...
            self.subtask_term_offset_ranges[eef_name] = [
                subtask_config.subtask_term_offset_range
                for subtask_config in eef_subtask_configs
            ]
    ...
    def _add_episode(self, episode: EpisodeData):
        ...
        if "datagen_info" in ep_grp["obs"]:
            ...
            subtask_term_signals_dict = ep_grp["obs"]["datagen_info"][
                "subtask_term_signals"
            ]
        else:
            raise ValueError(
                "Episode to be loaded to DatagenInfo pool lacks datagen_info annotations"
            )
        ...
        for eef_name in self.subtask_term_signal_names.keys():
            ...
            for subtask_index, subtask_term_signal_name in enumerate(
                self.subtask_term_signal_names[eef_name]
            ):
                if (
                    subtask_term_signal_name is None
                    or subtask_term_signal_name
                    not in ep_datagen_info_obj.subtask_term_signals
                ):
                    if isinstance(
                        self.env_cfg.subtask_configs[eef_name][subtask_index],
                        KeyPointSubTaskConfig,
                    ):
                        # KeyPointSubTaskConfig does not have a subtask_term_signal
                        subtask_term_ind = prev_subtask_term_ind + len(
                    ...
                    subtask_indicators = (
                        ep_datagen_info_obj.subtask_term_signals[
                    ...
                    def filter_short_ones(tensor, min_len=8):
                        tensor = tensor.clone()
                        # 找到1的起止位置
                        diff = torch.diff(
                            torch.cat(
                                [tensor.new_zeros(1), tensor, tensor.new_zeros(1)]
                            )
                        )
                    ...
                    if getattr(
                        self.env_cfg.subtask_configs[eef_name][subtask_index],
                        "slice_to_last_success_step",
                        False,
                    ):
                        # 将连续为1但长度不足8的片段置0

                        subtask_indicators = filter_short_ones(
                            subtask_indicators,
                            min_len=self.env_cfg.subtask_configs[eef_name][
                                subtask_index
                            ].subtask_continous_step,
                        )
                        end_ind = torch.where(subtask_indicators)[0][-1].item()
                    else:
                        if (
                            getattr(
                                self.env_cfg.subtask_configs[eef_name][subtask_index],
                    ...
                    subtask_term_ind = (
                        end_ind + 1
                    )  # increment to support indexing like demo[start:end]
            ...
            for i in range(1, len(eef_subtask_boundaries)):
                prev_max_offset_range = self.subtask_term_offset_ranges[eef_name][
                    i - 1
                ][1]
                assert (
