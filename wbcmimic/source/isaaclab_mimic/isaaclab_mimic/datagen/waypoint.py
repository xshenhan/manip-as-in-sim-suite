...
from isaaclab_mimic.utils.exceptions import (
    CollisionError,
...
class MultiWaypoint:
    ...
    async def execute(
        ...
        assert hasattr(success_term.func, "_subtask_history")
        success = bool(
            success_term.func(
                env, env_ids=torch.tensor([env_id]), **success_term.params
            )
        )
...
class MultiWaypointWbc(MultiWaypoint):
    ...
    async def execute(
        ...
        reached_this_waypoint = False
        env.q0s = None
        i = 0
        while not reached_this_waypoint and i < wbc_max_step:
            i += 1
            if mp:
                env.target_eef_pose_to_action_perpare(
                    target_eef_pose_dict, vel_multiplier, env_id
                )
            await asyncio.sleep(0)
            # calling user-defined env.target_eef_pose_to_action() with noise parameter is deprecated
            # (replaced by action_noise_dict)
            play_action, reached_this_waypoint = env.target_eef_pose_to_action(
                target_eef_pose_dict=target_eef_pose_dict,
                gripper_action_dict=gripper_action_dict,
                noise=max([waypoint.noise for waypoint in self.waypoints.values()]),
                env_id=env_id,
            ...
            # step environment
            if hasattr(env, "wbc_step_buf"):
                env.wbc_step_buf[env_id] = (i == 1) or (
                    getattr(env.cfg, "record_each_step", True)
                )
            if hasattr(env, "wbc_target_buf"):
                for eef_name, target_eef_pose in target_eef_pose_dict.items():
                    wbc_target = torch.zeros(
                        (1, 8), dtype=torch.float32, device=env.device
                    )
                    gripper_action = gripper_action_dict[eef_name]
                    wbc_target[0, :3] = target_eef_pose[:3, 3]
                    wbc_target[0, 3:7] = PoseUtils.quat_from_matrix(
                        target_eef_pose[:3, :3]
                    )
                    wbc_target[0, 7] = gripper_action
                    env.wbc_target_buf[eef_name][env_id] = wbc_target
            ...
            if success and success_break:
                break

        if (
            not reached_this_waypoint
            and env.cfg.mimic_config.episode_end_when_wbc_failed
        ):
            raise WbcStepOverMaxStepException()
