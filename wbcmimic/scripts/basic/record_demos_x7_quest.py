...
parser.add_argument(
    "--dataset_file",
    type=str,
    default="./datasets/test/temp.hdf5",
...
try:
    from isaaclab_mimic.utils.robots.ros_robots_interface import RosX7Interface
except:
    print("Warning: RosX7Interface not found.")
    RosX7Interface = None
...
def main():
    ...
    ee_reset_pose_l = torch.tensor(
        [0.4195, 0.3422, 0.5123, 1, 0, 0, 0], device=env.device
    )
    ee_reset_pose_r = torch.tensor(
        [0.4195, -0.3422, 0.5123, 1, 0, 0, 0], device=env.device
    )
    ...
    if RosX7Interface is not None:
        ros_interface = RosX7Interface(env)
    with contextlib.suppress(KeyboardInterrupt) and torch.inference_mode():
        while True:
            ...
            actions[:, 0] = lift_des
            actions[:, -3] = 1 * vr_actions["forward_speed"]
            actions[:, -2] = 1 * vr_actions["side_speed"]
            actions[:, -1] = 1 * vr_actions["yaw_speed"]
